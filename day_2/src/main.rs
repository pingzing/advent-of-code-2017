...
fn main() {
    checksum_rows(INPUT_STR.lines());
    checksum_rows_with_division(INPUT_STR.lines());
}

fn checksum_rows(input_str: str::Lines) {
    let mut total_checksum = 0u32;

    for line in input_str {        
        let digits = line.split(',').map(|x| u32::from_str(x).unwrap());
        let (smallest, largest) = max_and_min(digits);
        let difference = largest - smallest;
        total_checksum = total_checksum + difference;
    }

    println!("Total checksum is: {}", total_checksum);
}

fn checksum_rows_with_division(input_str: str::Lines) {
    let mut total_checksum = 0u32;
    ...
    for line in input_str {
        ...
        for digit in &digits {
            for second_digit in &digits {
                if second_digit != digit && (digit % second_digit) == 0 {
                    total_checksum += digit / second_digit;
                }
            }
        }
    }

    println!("Total checksum for division is: {}", total_checksum);
}

fn max_and_min<I>(iterator: I) -> (u32, u32)
    where I: IntoIterator<Item=u32> {        
        let mut smallest = u32::max_value();
        let mut largest = u32::min_value();
